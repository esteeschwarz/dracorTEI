...
## please note:
if you process copyrighted material which is not to be made available in the provided dracor preview (which allows for downloading the text) you must reload the app once after finish you work here, this will overwrite the database content of the preview.xml on the dracor side with sample content. keep that in mind...
...
## limitations
- its not yet possible to markup verse (\<l> in the TEI), the standard text tag applied is \<p>.
- problem with headers like `ANDEN AKT, SCENE 1` which dont ezd tag the second part so theres no divs for the scenes created. we'll fix that... / fixed.

## fixings
- verse: you can now have verse drama which will be tagged with \<l> instead \<p> speech acts. still you have to pre markup the transcript for that like:
...
~and the haifish
he has teeth
and he shows them not
~now how was that song?
```

- as you see you have to switch from verse to prose in your transcript and exit verse with `~`. If you just set `~` once at the beginning, then the whole play with treated as verse drama.
